...
const RedisBrowser = memo(({ uniqueData, className }: IViewAllTableProps) => {
  ...
  const tableColumns: ColumnsType<IRedisKeyItem> = [
      ...
      render: (value, record) => (
        <div
          className={classnames(styles.tableCell, {
            [styles.activeTableCell]: activeKey === record.keyName,
          })}
        >
          {value}
        </div>
    ...
    },
    {
      ...
      render: (value, record) => (
        <div
          className={classnames(styles.tableCell, styles.valueCell, {
            [styles.activeTableCell]: activeKey === record.keyName,
          })}
          title={value || ''}
        >
  ...
  const simpleColumns = (setter: React.Dispatch<React.SetStateAction<ISimpleRow[]>>): ColumnsType<ISimpleRow> => [
    {
      ...
      render: (_, row) => (
        <Input
          value={row.value}
          onChange={(e) =>
            setter((prev) => prev.map((item) => (item.id === row.id ? { ...item, value: e.target.value } : item)))
          }
          size="small"
        />
      ),
  ...
  const zsetColumns: ColumnsType<IZSetRow> = [
    {
      ...
      render: (_, row) => (
        <Input
          ...
          onChange={(e) => {
            setZsetRows((prev) =>
              prev.map((item) => (item.id === row.id ? { ...item, member: e.target.value } : item)),
            );
          }}
          size="small"
        />
